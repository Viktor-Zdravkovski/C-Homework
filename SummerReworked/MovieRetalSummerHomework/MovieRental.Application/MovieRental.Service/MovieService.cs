...
namespace MovieRental.Service
{
    public class MovieService : IMovieService
    {
        ...
        {
            // Assuming you have a way to get movies rented by a user
            var rentedMovies = _movieRepository.GetAll()
                .Where(m => !m.IsAvailable) // Adjust if necessary for your data model
                .Select(m => new MovieDto
                {
                    Id = m.Id,
                    Title = m.Title,
                    Genre = m.Genre, // Assume Genre is a property of the Movie entity
                    Language = m.Language, // Assume Language is a property of the Movie entity
        ...
        public void RentMovie(int movieId, string userName)
        {
            ...
            //_movieRepository.Update(movie); // Update the movie status in the database
            ...
            // Mark the movie as rented
            movie.IsAvailable = false;
            movie.Quantity -= 1; // Update the quantity if needed

            _movieRepository.Update(movie); // Update the movie status in the database

            // Add rental tracking here, e.g., store in a static list or another data structure
            // For example: AddRentalRecord(movieId, userName);
        }
